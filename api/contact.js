...
module.exports = async (req, res) => {
  ...
  const to   = "homeperfect2022@yahoo.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Home Perfect <leads@homeperfectllc.com>";

  const subject = `New lead from homeperfectllc.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on homeperfectllc.com`;
  const html = `
<h2>New lead from Home Perfect</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on homeperfectllc.com</p>`;
